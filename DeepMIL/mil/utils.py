...
from slide.utils import read_h5, read_h5_features, read_h5_coords
...
def prep_wsi(path: str | Path, args) -> dict[str, Any]:
    ...
    """
    img_key = "normalized" if getattr(args, "use_normalized", False) else "features"
    _, array = read_h5(str(path), key=img_key)
    ...
    slide_size = np.asarray(slide_size, dtype=np.float32,)
...
def mean_mcdo_std(
    ...
    """
    Average MCDO variability across ensemble members without warning when
    MCDO was disabled for every model.
    """
